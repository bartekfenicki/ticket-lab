<template>
  <div class="p-4">
    ...
    <div class="mb-4">
      <label class="block mb-1 font-medium">Reservation Option Type <span class="font-thin text-gray-400">(select a Type to display info)</span></label>
      <select v-model="selectedOptionTypeId" @change="loadVariants" class="border rounded-lg p-2 w-full">
    ...
    <div class="mb-6 border p-4 rounded-lg shadow-sm bg-white">
      <h3 class="text-xl font-medium mb-3">{{ editingId ? "Edit Variant" : "Create Variant" }}</h3>
    ...
    <div class="bg-white shadow-md rounded-lg overflow-x-auto">
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="v in variantsStore.variants" :key="v.id" class="hover:bg-gray-50 transition">
            <td class="px-4 py-3">{{ v.name }}</td>
            <td class="px-4 py-3">{{ v.price }}</td>
            <td class="px-4 py-3">{{ v.pricing_type }}</td>
            <td class="px-4 py-3">
              <span :class="v.active ? 'text-green-600 font-semibold' : 'text-gray-500'">
                {{ v.active ? "Yes" : "No" }}
              </span>
            </td>
          ...
          <tr v-if="variantsStore.variants.length === 0">
            <td colspan="5" class="px-4 py-6 text-center text-gray-500">No variants found.</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const loadVariants = async () => {
  if (selectedOptionTypeId.value) {
    ...
    await variantsStore.fetchVariantsByOptionType(selectedOptionTypeId.value);
  }
};
...
const handleSubmit = async () => {
  form.value.reservation_option_type_id = selectedOptionTypeId.value!;
  if (editingId.value) {
    await variantsStore.updateVariant(editingId.value, form.value);
  } else {
    await variantsStore.createVariant(form.value);
  }
  resetForm();
  await loadVariants();
};
...
const deleteVariant = async (id: number) => {
  if (confirm("Delete this variant?")) {
    await variantsStore.deleteVariant(id);
  }
};
