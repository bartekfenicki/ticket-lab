<template>
  <div class="max-w-6xl mx-auto p-6 bg-white rounded-xl shadow-md space-y-8">
    <h2 class="text-2xl font-bold text-indigo-700">🎟 Ticket Types Management</h2>
    ...
    <form @submit.prevent="addTicketType" class="grid grid-cols-1 md:grid-cols-2 gap-4 items-end border-b pb-6">
      <div>
        ...
        <input v-model="newTicket.name" type="text" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-indigo-500" required />
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Price</label>
        <input v-model.number="newTicket.price" type="number" min="0" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-indigo-500" required />
      </div>
      <div class="md:col-span-2">
        <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
        <textarea v-model="newTicket.description" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-indigo-500" rows="2"></textarea>
      </div>
      <div class="flex gap-4 items-center">
        <label class="flex items-center gap-2">
          <input type="checkbox" v-model="newTicket.is_special_event" class="accent-indigo-600 w-5 h-5"/>
          Special Event
        </label>
        <label class="flex items-center gap-2">
          <input type="checkbox" v-model="newTicket.active" class="accent-indigo-600 w-5 h-5"/>
          Active
        </label>
      </div>
      <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-6 py-2 rounded-lg transition">Add Ticket Type</button>
    </form>
